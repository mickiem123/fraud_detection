...
class Preprocessor():
    ...
    def get_TERMINAL_ID_characteristic(self):
        ...
        TX_DELAY = self.df.groupby("TERMINAL_ID").rolling(f"{self.config.delay}d")["TX_FRAUD"].count()
        for size in self.config.windows_size:
            FRAUD_DELAY_WINDOW = self.df.groupby("TERMINAL_ID").rolling(f"{self.config.windows_size + self.config.delay}d",on="TX_DATETIME")["TX_FRAUD"].sum()
            TX_DELAY_WINDOW = self.df.groupby("TERMINAL_ID").rolling(f"{self.config.windows_size + self.config.delay}d",on="TX_DATETIME")["TX_FRAUD"].count()
